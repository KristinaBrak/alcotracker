...
const updateStoreProducts = async (store: Store, products: readonly ApiProduct[]) => {
  ...
  Promise.all(
    products.map(async ({ category, image, link, name, price, alcVolume, volume }) => {
...
const refreshMaterializedView = async (materializedView: string) => {
  ...
  await queryRunner
    .query(`REFRESH MATERIALIZED VIEW ${materializedView}`)
    .catch(async () => await queryRunner.release());
...
export const executeStoreRunner = async () => {
  ...
  await updateStores();
  await Promise.all(stores.map(fetchAndUpdateStoreProducts)).catch(logger.error);
